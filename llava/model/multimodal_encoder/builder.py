...
from .voxel_encoder import SecondVoxelTower, PillarVoxelTower
...
def build_voxel_tower(voxel_tower_cfg, **kwargs):
    voxel_tower = getattr(voxel_tower_cfg, 'mm_voxel_tower', getattr(voxel_tower_cfg, 'voxel_tower', None))
    if voxel_tower.endswith('SecondVoxelnet'):
        return SecondVoxelTower(args=voxel_tower_cfg, **kwargs)
    if voxel_tower.endswith('Pointpillars'):
        return PillarVoxelTower(args=voxel_tower_cfg, **kwargs)
    raise ValueError(f'Unknown voxel tower: {voxel_tower}')