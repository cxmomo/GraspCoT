...
def interpolate_feat_up(feat, method="nearest"):
    BV, h, w, C = feat.shape
    # xyz_ = torch.nn.functional.interpolate(
    #     xyz.reshape(B*V, H, W, 3).permute(0, 3, 1, 2), size=(h, w),
    #     mode=method).permute(0, 2, 3, 1).reshape(B, V, h, w, 3) 
    ...
    feat_up = torch.nn.functional.interpolate(
        feat.view(BV, h, w, C).permute(0, 3, 1, 2).contiguous(), size=(56, 56),
        mode=method).permute(0, 2, 3, 1).contiguous()

    return feat_up

def interpolate_xyz_down(xyz, method="nearest"):
    B, V, H, W, _ = xyz.shape

    xyz_new = torch.nn.functional.interpolate(
        xyz.view(B*V, H, W, -1).permute(0, 3, 1, 2).contiguous(), size=(H//6, W//6),
        mode=method).permute(0, 2, 3, 1).reshape(B, V, H//6, W//6, 3).contiguous()
