...
from .multimodal_encoder.builder import build_vision_tower, build_video_tower, build_voxel_tower
from .multimodal_projector.builder import build_vision_projector

from llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, GRASP_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, LOC_TOKEN_INDEX
...
class LlavaMetaModel:
    ...
    def get_voxel_tower(self):
        voxel_tower = getattr(self, 'voxel_tower', None)
        if type(voxel_tower) is list:
            voxel_tower = voxel_tower[0]
        return voxel_tower
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        voxel_tower = model_args.voxel_tower
        ...
        self.config.mm_voxel_tower = voxel_tower
        if voxel_tower is not None:
            if self.get_voxel_tower() is None:
                voxel_tower = build_voxel_tower(model_args)

                if fsdp is not None and len(fsdp) > 0:
                    self.voxel_tower = [voxel_tower]
                else:
                    self.voxel_tower = voxel_tower
            else:
                if fsdp is not None and len(fsdp) > 0:
                    voxel_tower = self.voxel_tower[0]
                else:
                    voxel_tower = self.voxel_tower
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_voxel_tower(self):
        return self.get_model().get_voxel_tower()
    ...
    def encode_images(self, images):  
        ...
        image_features = self.compress_proj(image_features.permute(0, 2, 1)).permute(0, 2, 1)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        images, pure_imgs, depths, poses, intrinsics, lengths, pcs=None, clicks=None, image_sizes=None
    ):
        ...
        if images.ndim == 5 and depths is not None:
            ...
            if batch_offset is not None:
                image_features = []
                idx = 0
                for i_b, b in enumerate(batch_offset):
        ...
        if pure_imgs is not None:
            if type(image_features) is list:
                for tt in range(len(image_features)):
                    image_features[tt] = torch.cat((pure_img_features[tt], image_features[tt]), dim=0)
            elif type(image_features) is torch.Tensor:
                image_features = torch.cat((pure_img_features, image_features), dim=1)
            ...
            for i in range(num_specials + 1):  # num_images = 1? [0, 1]
                ...
                if i < num_specials:
                    ...
                    if special_token == IMAGE_TOKEN_INDEX:
                        ...
                        cur_new_labels.append(torch.full((cur_image_features.shape[0],), GRASP_TOKEN_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
        ...
        # b = torch.isnan(new_input_embeds).any()
        new_input_embeds = torch.nan_to_num(new_input_embeds)
