...
def normalize_bbox(bboxes):
    ...
    # out = torch.cat([x_rot.sin(), x_rot.cos(), y_rot.sin(), y_rot.cos(), z_rot.sin(), z_rot.cos(), cx, cy, cz, w], dim=-1)
    out = torch.cat([x_rot, y_rot, z_rot, cx, cy, cz, w], dim=-1)
...
class HungarianAssigner3D(BaseAssigner):
    ...
    def assign(self,
        ...
        reg_cost = self.reg_cost(bbox_pred[:, :self.code_size], normalized_gt_bboxes[:, :self.code_size])
        ...
        # assigned_labels[matched_row_inds] = gt_labels[matched_col_inds, 0]
        return AssignResult(
            num_gts, assigned_gt_inds, None, labels=assigned_labels)
