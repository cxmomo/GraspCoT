...
class ModelArguments:
    ...
    voxel_tower: Optional[str] = field(default=None)
    tune_voxel_tower: bool = field(default=False)
    # ===================================================================
...
class DataArguments:
    ...
    dialogue: bool = field(default=False)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        grasps, grasp_labels = tuple([instance[key] for instance in instances]
                                  for key in ("grasps", "gs_labels"))
        ...
        if 'pure_img' in instances[0]:
            pure_imgs = [instance['pure_img'] for instance in instances]
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = GraspcotDataset_Train(data_args.data_path[0], tokenizer=tokenizer)
...
def train(attn_implementation=None):
    ...
    if local_rank == 0:
        ...
        work_dir = os.path.join(training_args.logging_dir)
        ...
        logging.basicConfig(level=logging.INFO,
                            filename=os.path.join(work_dir, "train.log"),
                            datefmt='%Y/%m/%d %H:%M:%S',
    ...
    if training_args.bits in [4, 8]:
        ...
        bnb_model_from_pretrained_args.update(dict(
            ...
            quantization_config=BitsAndBytesConfig(
                load_in_4bit=training_args.bits == 4,
                load_in_8bit=training_args.bits == 8,
                llm_int8_skip_modules=["mm_projector", "voxel_tower"],
    ...
    trainer = LLaVATrainer(model=model,
                    ...
                    args=training_args,
                    logger=logger,
                    use_dialog=model.config.use_dialogue,
                    **data_module)

    if list(pathlib.Path(training_args.output_dir).glob("herecheckpoint-*")):
