...
from llava.constants import IMAGE_TOKEN_INDEX, LOC_TOKEN_INDEX, UNSUPERVISED_TOKEN_INDEX
...
def tokenizer_special_token_v2(prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, box_token_index=LOC_TOKEN_INDEX, unsupervised_token_index=UNSUPERVISED_TOKEN_INDEX, return_tensors=None):
    import re
    separators = re.compile(r'(<image>|<boxes>|<unsupervised>)')
    input_ids = []
    offset = 0
    prompt_chunks = separators.split(prompt)  # ['"This is a test ', '<image>', ' with an image ', '<boxes>', ' and some boxes ', '<image>', ' another image']
    first_chunk = tokenizer(prompt_chunks[0]).input_ids
    if first_chunk[0] == tokenizer.bos_token_id:
        ...
        input_ids.append(first_chunk[0])
    for prompt_chunk in prompt_chunks:
        if prompt_chunk not in ['<image>', '<boxes>', '<unsupervised>']:
            input_ids.extend(tokenizer(prompt_chunk.rstrip()).input_ids[offset:])
        elif prompt_chunk == '<image>':
            input_ids.extend([image_token_index])
        elif prompt_chunk == '<boxes>':
            input_ids.extend([box_token_index])
        elif prompt_chunk == '<unsupervised>':
            input_ids.extend([unsupervised_token_index])
        else:
            raise NotImplementedError
    if return_tensors is not None:
        if return_tensors == 'pt':
